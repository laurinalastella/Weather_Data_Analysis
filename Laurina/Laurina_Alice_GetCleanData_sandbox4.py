...
def get_latt_long(country_input="countryNULL", query_input="queryNULL"):
...
# Can't fix by using these variables.)
    ...
    country_input_var = country_input
    query_input_var = query_input
    # Coder's choice of replacement location:
    country_input_var_replacement = "US"
    query_input_var_replacement = "94123"   #Union Square, SF, CA, US.

    # If user input = ():
    # Assign new values to both:
    if country_input_var == "countryNULL" and query_input_var == "queryNULL":
        country_input_var = country_input_var_replacement
        query_input_var = query_input_var_replacement
    # If user input = ('text') with no comma:
    # Force Country and Query to agree, 
    # so also assign new values to both:
    elif country_input_var != "countryNULL" and query_input_var == "queryNULL":
        country_input_var = country_input_var_replacement
        query_input_var = query_input_var_replacement
    ...
    params = urllib.parse.urlencode({
        ...
        'country': country_input_var,
        'query': query_input_var,
    ...
    try:
        ...
        return(f"Error in geocoding json.")
...
def get_hourly_weather_url(lattitude, longitude, start_year=2023, end_year=2023):
    url_snip_hr = {}
