...
# get_latt_long gets(country, city) latitude and longitude 
# for a given city and country from positionstack.com
# ### variables could use renaming:
#    country means country,
#    city means city

def get_latt_long(country="countryNULL", city="cityNULL"):
# Default values are assigned in function definition,
# which will take the place of missing inputs.

# (If first argument is missing, but comma is present, like (,)
# whether or not second argument is present: 
# Result = "SyntaxError: invalid syntax"
# Can't fix by using these variables.
# Also can't handle with try...except.)

    # Raise custom errors:
    # Verify that country and city are strings, not numbers.
    if isinstance(country, str):
        country_var = country
    else:
        raise Exception ("Country input is a number. Please enter a 2-letter country code.")
    if isinstance(city, str):
        city_var = city
    else:
        raise Exception ("City input is a number. Please enter a city name or postal code.")

    # Catching and fixing missing inputs:
    # Create variables to contain new values.
    country_var = country
    city_var = city
    # Coder's choice of replacement location:
    country_var_replacement = "US"
    city_var_replacement = "94720"   #Berkeley University, CA, US.

    # If user input = ():
    # Assign new values to both:
    if country_var == "countryNULL" and city_var == "cityNULL":
        country_var = country_var_replacement
        city_var = city_var_replacement
    # If user input = ('text') with no comma:
    # Force Country and City to agree, 
    # so also assign new values to both:
    elif country_var != "countryNULL" and city_var == "cityNULL":
        country_var = country_var_replacement
        city_var = city_var_replacement
    # Neither are missing, use user input.
    else:
        country_var = country
        city_var = city
    ...
    # Try...Except for handling inputs 
    ...
    try:
        ...
        return(f"KeyError in geocoding json.")
    except TypeError:
        return(f"TypeError in geocoding json.")
    except IndexError:
        return(f"IndexError in geocoding json.")
...
# Error handling for stats_dict,
# Check for values missing.
class StatsExceptionError(Exception):
    pass
...
def bool_to_stats(daf):
    ...
    try:
        ...
        return(f"RuntimeWarning in stats_dict.")
    except KeyError:
        return(f"KeyError in stats_dict.")
    except TypeError:
        return(f"TypeError in stats_dict.")
    except IndexError:
        return(f"IndexError in stats_dict.")
    except BaseException as e:
        return(e)
